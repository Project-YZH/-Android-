...
//2024.11.5//
pool<std::pair<RTLIL::IdString, int>> mod_port_info;//store the information of mod input and output. <mod_num, index of the last input>;
string top_mod_name;//input the top mode name from shell (correspondence with the later "hierarchy -top" pass)
...
static RTLIL::Module *process_module(RTLIL::Design *design, AstNode *ast, bool defer, AstNode *original_ast = NULL, bool quiet = false)
{
	...
	if (!defer)
	{
		...
		while (ast->simplify(!flag_noopt, false, false, 0, -1, false, false)) { } //SIMPLIFY
	...
	//2024.11.5//get the mod_cell port(input or output) information[expect the top_mod]
	///*[2024.11.28]
	if(top_mod_name != log_id(current_module->name)){
		int index = 0;
		for(auto mod_wires : current_module->wires_){
			if(mod_wires.second->port_input == true)
				index ++;
		}
		mod_port_info.insert(std::pair<RTLIL::IdString, int>(current_module->name, index));
	}
	else
		top_mod = current_module;
	//[2024.11.28]*/
///////////////////////////////////////////////////////////EASY VERSION////////////////////////////////////////
	//// traverse the mod until no new taint_signal
	/***ID(top_mod)***/
	/*2024.10.5
	if(current_module->name == ID(EncCore)){
		//// collect the initial taint_input;
		int taint_input_num;
		log("Enter the number of taint_input : \t");
		std::cin >> taint_input_num;

		string taint_signal;
		std::set<string> taint_signal_set;
		for(int i=0; i < taint_input_num; i++){
			log("the %d taint input: \t", i);
			std::cin >> taint_signal;
			taint_signal_set.insert(taint_signal);
		}
		int taint_signal_num = 0;
		while(taint_signal_set.size() > taint_signal_num){
			/// traverse the process of mod
			for(auto proc_it : current_module->processes){
				//Determine if process needs to be processed.
				if(proc_it.second->get_bool_attribute(ID(taint_area)) == true)
					continue;
				//Initialize, collect process input and output information.
				//Determine whether process belongs to the taint area.
				RTLIL::Process* proc = proc_it.second;
				if(taint_signal_set.count(log_id(proc_it.second->root_case.switches.front()->signal.chunks().front().wire->name)) != 0){
					proc_it.second->set_bool_attribute(ID(taint_area), true);
					taint_signal_set.insert(log_id(proc_it.second->syncs.front()->signal.chunks().front().wire->name));
				}
			}

			///traverse the cell of mod
			for(auto cell_it : current_module->cells_){
				if(cell_it.second->get_bool_attribute(ID(taint_area)) == true)
					continue;
				string out_name;
				for(auto sig_it : cell_it.second->connections_){
					if(sig_it.second.is_fully_const())
						continue;
					string sig_name = log_id(sig_it.second.chunks().front().wire->name);
					//Get the output information of the cell
					if(sig_it.first == ID(Y) || sig_it.first == ID($2))
						out_name = sig_name;
					//Determine whether the input of cell containts taint information
					if(taint_signal_set.count(sig_name) != 0){
						cell_it.second->set_bool_attribute(ID(taint_area), true);
						taint_signal_set.insert(out_name);
						break;
					}
				}
			}

			////traverse the connections of mod
			for(auto conn_it : current_module->connections_){
				string in_name = log_id(conn_it.second.chunks().front().wire->name);
				string out_name = log_id(conn_it.first.chunks().front().wire->name);
				if(taint_signal_set.count(in_name) != 0)
					taint_signal_set.insert(out_name);
			}


			if(taint_signal_num == taint_signal_set.size())
				break;
			taint_signal_num = taint_signal_set.size();
		}
	}
	2024.11.5*/
///////////////////////////////////////////////////////////FULL VERSION//////////////////////////////////////////////////////////

	//// traverse the mod from the top one, until all modules are processed.
	/***ID(top_mod)***/
	//after the "hierarchy -top /top_mod", the top mod has the attribute \top 1;
	/*2024.10.10
	if(current_module->name == ID(EncCore)){ //processing starts with the topmost module.

		////1. collect the initial taint_input; aka constraint condition.
		int taint_input_num;
		log("Enter the number of taint_input : \t");
		std::cin >> taint_input_num;
		...
		////2. Stopping the loop when there is no new taint signal means that there are no new taint areas.
		while(taint_signal_set.size() > taint_signal_num){
			/// 2.1 traverse the process of mod
			pool<string> proc_input;
			pool<string> proc_output;
			for(auto proc_it : current_module->processes){
				//Determine if process needs to be processed.
				if(proc_it.second->get_bool_attribute(ID(taint_area)) == true)
					continue;
				...
				if(proc_it.second->get_bool_attribute(ID(pro_init)) != true){
					//get the output
					for(auto output_syn : proc_it.second->syncs)
						for(auto output_act : output_syn->actions)
							proc_output.insert(log_id(output_act.first.chunks().front().wire->name));
					//get the input
					for(auto input_pro : proc_it.second->root_case.switches){
						//switch input
						proc_input.insert(log_id(input_pro->signal.chunks().front().wire->name));
						//assign input
						for(auto input_case : input_pro->cases){
							for(auto input_act: input_case->actions){
								if(input_act.second.is_fully_const() == false)
								proc_input.insert(log_id(input_act.second.chunks().front().wire->name));
							}
						}
					}
					//collect the inout information of process.
					proc_it.second->set_strpool_attribute(ID(proc_input), proc_input);
					proc_it.second->set_strpool_attribute(ID(proc_output), proc_output);

					proc_it.second->set_bool_attribute(ID(pro_init), true);
				}

				//Determine whether process belongs to the taint area.[complex_version: like RSA case]
				proc_input = proc_it.second->get_strpool_attribute(ID(proc_input));
				proc_output = proc_it.second->get_strpool_attribute(ID(proc_output));
				for(auto it_input : proc_input){
					if(taint_signal_set.count(it_input) != 0){
						for(auto it_output : proc_output)
							taint_signal_set.insert(it_output);
						proc_it.second->set_bool_attribute(ID(taint_area), true);
						break;
					}
				}
		...
		bool nolatches, bool nomeminit, bool nomem2reg, bool mem2reg, bool noblackbox, bool lib, bool nowb, bool noopt, bool icells, bool pwires, bool nooverwrite, bool overwrite, bool defer, bool autowire)
{
	...
	//2024.11.6//get the top mod of the design
	///*[2024.11.28]
	log("Enter the name of top mod: \t");
		std::cin >> top_mod_name;
	//[2024.11.28]*/
	for (AstNode *child : current_ast->children)
	{
		if (child->type == AST_MODULE || child->type == AST_INTERFACE)
		{
			...
			process_module(design, child, defer_local);//enter the mod transform (from AST to RTLIL)
			current_ast_mod = nullptr;
		}
	...
	//2024.11.6//SHIFT start work from the top mod
	////1.1 collect the initial taint_input; aka constraint condition.
	///*[2024.11.28]
	int taint_input_num;
	log("Enter the number of taint_input : \t");
	std::cin >> taint_input_num;
	...
	int taint_signal_num = 0; //for 2.1 while() loop anchor
	
	////1.2 collect the initial untaint_input; aka constraint condition.
	int untaint_input_num;
	log("Enter the number of untaint_input: \t");
	std::cin >> untaint_input_num;

	string untaint_signal;
	std::set<string> untaint_signal_set;
	for(int i=0; i< untaint_input_num; i++){
		log("the %d untaint input: \t", i);
		std::cin >> untaint_signal;
		untaint_signal_set.insert(untaint_signal);
	}
	int untaint_signal_num = 0;
	////1.end the taint and untaint signal in the <taint_signal_set> and the <untiant_signal_set>
	
	////2. Stopping the loop when there is no new taint signal means that there are no new taint areas.
	bool port_opt_flag = false;
	bool area_opt_flag = false;
	while(taint_signal_set.size() >= taint_signal_num){

		taint_signal_num = taint_signal_set.size();
		if(area_opt_flag == true){
			untaint_signal_num = untaint_signal_set.size();}
	////2.1 traverse the process of mod
		pool<string> proc_input;
		pool<string> proc_output;
		for(auto proc_it : top_mod->processes){
			//2.1.1 Determine if process had been processed.
			if(proc_it.second->get_bool_attribute(ID(taint_area)) == true)
				continue;

			//2.1.2 Initialize, collect process input and output information.
			if(proc_it.second->get_bool_attribute(ID(pro_init)) != true){
				//2.1.2.1 get the output of process, abstract as a cell output
				for(auto output_syn : proc_it.second->syncs)
					for(auto output_act : output_syn->actions){
						if(output_act.second.chunks().front().wire != NULL) //good point
							proc_output.insert(log_id(output_act.first.chunks().front().wire->name));
					}
				
				//2.1.2.2 get the input of process, abstract as a cell input
				
				if(proc_it.second->root_case.switches.size() == 0){
					for(auto action_it : proc_it.second->root_case.actions){
						if(action_it.second.is_fully_const() == false)
							proc_input.insert(log_id(action_it.second.chunks().front().wire->name));
					}
				}	
				else{
				vector<RTLIL::SwitchRule *> sw_it_set;
				sw_it_set.push_back(proc_it.second->root_case.switches.front());
				proc_input.insert(log_id(proc_it.second->root_case.switches[0]->signal.chunks().front().wire->name));
				for(auto sw_it : sw_it_set){
		
					for(auto case_it : sw_it->cases){

						if(case_it->actions.size() != 0){
							for(auto action_it : case_it->actions){
								if(action_it.second.is_fully_const() == false)
								proc_input.insert(log_id(action_it.second.chunks().front().wire->name));
							}
						}
						if(case_it->switches.size()!=0){
							for(auto switch_it : case_it->switches){
								proc_input.insert(log_id(switch_it->signal.chunks().front().wire->name));
								sw_it_set.push_back(case_it->switches.front());
							}
						}
					}
				}
				...
				for(auto input_pro : proc_it.second->root_case.switches){
					...
					for(auto input_case : input_pro->cases){
						...
						// if input_case contain switches, return to 1624;
					}
				}
				*/
				//2.1.2.3 collect the inout information of process.
				proc_it.second->set_strpool_attribute(ID(proc_input), proc_input);
				proc_it.second->set_strpool_attribute(ID(proc_output), proc_output);
				proc_it.second->set_bool_attribute(ID(pro_init), true);
			}
			if(area_opt_flag == false){
				//2.1.3 Determine whether process belongs to the taint area.[complex_version: like RSA case]
				proc_input = proc_it.second->get_strpool_attribute(ID(proc_input));
				proc_output = proc_it.second->get_strpool_attribute(ID(proc_output));
				for(auto it_input : proc_input){
					if(taint_signal_set.count(it_input) != 0){
			...
			/////2024.7.1//port_opt
			if(area_opt_flag == true && proc_it.second->get_bool_attribute(ID(taint_area)) != true){
				//2.1.4 Determine whether process belongs to the untaint area.[complex_version: like RSA case]
				proc_output = proc_it.second->get_strpool_attribute(ID(proc_output));
				for(auto it_output : proc_output)
					untaint_signal_set.insert(it_output);
				proc_it.second->set_bool_attribute(ID(untaint_area), true);
			}
		}
	////2.2 traverse the cell of mod [also contain the mod_cell]
		
		for(auto cell_it : top_mod->cells_){
			if(cell_it.second->get_bool_attribute(ID(taint_area)) == true && area_opt_flag == false)
				continue;
			//init: get the boundary of input and output, cell is "\Y", mod is "$index"
			string cell_type = log_id(cell_it.second->type);
			int index = 0;
			if(cell_type.at(0) != '$'){ //normal cell type "$XXX", mod cell type "\XXX"
				for(auto mod_type : mod_port_info){
					if(cell_type == log_id(mod_type.first)){
						index = mod_type.second;
						break;
					}
				}
			}////good idea: we can store the index information into attribute, need not to travel mod_port_info every time.
			...
			if(index == 0){
				...
				for(auto sig_it : cell_it.second->connections_){
					if(sig_it.second.is_fully_const()){
						if(area_opt_flag == false)
							continue;
						else if(area_opt_flag == true){
							cell_it.second->set_string_attribute(ID(const_opt), log_id(sig_it.first));
							continue;
						}
					}
					...
					string sig_name = log_id(sig_it.second.chunks().front().wire->name);
					//Get the output information of the cell
					if(sig_it.first == ID(Y)){////good idea: we can use "continue" when detect the Y
						out_name = sig_name;
						continue;
						}
					//Determine whether the input of cell containts taint information
					if(area_opt_flag == false && taint_signal_set.count(sig_name) != 0){
						cell_it.second->set_bool_attribute(ID(taint_area), true);
						taint_signal_set.insert(out_name);
						break;
					}
					if(area_opt_flag == true && untaint_signal_set.count(sig_name) != 0){
						
						if(cell_it.second->get_bool_attribute(ID(taint_area)) != true){
							untaint_signal_set.insert(out_name);
							cell_it.second->set_bool_attribute(ID(untaint_area), true);
						}
						else{
							cell_it.second->set_string_attribute(ID(port_opt), log_id(sig_it.first));
						}
					}
			...
			if(index > 0){
				...
				for(auto sig_it : cell_it.second->connections_){
					if(sig_it.second.is_fully_const())
						continue;
					string sig_name = log_id(sig_it.second.chunks().front().wire->name);
					string port_index_string = log_id(sig_it.first);
					int port_index = port_index_string.at(1) - 48;
					if(index < port_index){
						out_name.insert(out_name.begin(), sig_name);
					}
					if(index >= port_index){
						if(area_opt_flag == false && taint_signal_set.count(sig_name) != 0){
							cell_it.second->set_bool_attribute(ID(taint_area), true);
							for(auto it : out_name){
								taint_signal_set.insert(it);
							}
						}
						if(area_opt_flag == true && untaint_signal_set.count(sig_name) != 0){
							if(cell_it.second->get_bool_attribute(ID(taint_area)) != true){
								for(auto it : out_name)
									untaint_signal_set.insert(it);
								cell_it.second->set_bool_attribute(ID(untaint_area), true);
							}
						else
							cell_it.second->set_string_attribute(ID(port_opt), log_id(sig_it.first));
	...
	////2.3 traverse the connections of mod
		for(auto conn_it : top_mod->connections_){
			string in_name = log_id(conn_it.second.chunks().front().wire->name);
			string out_name = log_id(conn_it.first.chunks().front().wire->name);
			if(taint_signal_set.count(in_name) != 0)
				taint_signal_set.insert(out_name);
		}
	////2.4 if no new taint signal, stop the while
		if(taint_signal_num == taint_signal_set.size()){
			area_opt_flag = true;
			if(untaint_signal_num == untaint_signal_set.size()){
				port_opt_flag = true;
				break;
			}
		}
