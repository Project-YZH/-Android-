	...
	void handle_memory(Mem &mem)
	{
		...
		for (int i = 0; i < GetSize(mem.rd_ports); i++)
		{
			...
			for (int j = 0; j < abits - port.wide_log2; j++)
			{
				...
				{
					RTLIL::Cell *c = module->addCell(genid(mem.memid, "$rdmux", i, "", j, "", k), ID($mux));//transform memrd to muxtree;
					...
					///*[2024.11.28]
					if(i==0 && j==abits-port.wide_log2-1)
						c->set_bool_attribute(ID(mem_first), true);
					//[2024.11.28]*/
