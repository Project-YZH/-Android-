...
//**********************2025/2/25**************************/
//****************PGtest_roollback*************************/
// Path rollback searcher
class PathRollbackSearch {
private:
    vector<PathNode> current_path;   // 当前执行路径
    vector<string> current_constraints; // 当前约束集合
    bool vulnerability_triggered = false;

public:

    void add_path_node(int block_id, const vector<string>& constraints) {
        current_path.emplace_back(block_id, constraints);
        current_constraints.insert(current_constraints.end(), constraints.begin(), constraints.end());
    }
    ...
    //The core of the path rollback search algorithm
    vector<string> rollback_search() {
        vector<string> new_constraints;
        
        for (int i = current_path.size() - 1; i >= 0; --i) {
            auto& node = current_path[i];

            if (try_alternative_branch(node, i, new_constraints)) {
...
private:
    bool try_alternative_branch(const PathNode& node, int index, vector<string>& output) {

        vector<string> original_constraints = node.constraints;

        vector<string> negated_constraints = negate_constraints(original_constraints);
        

        vector<string> temp_constraints;
        for (int j = 0; j < index; ++j) {
            temp_constraints.insert(temp_constraints.end(), 
                                  current_path[j].constraints.begin(),
                                  current_path[j].constraints.end());
        }
        temp_constraints.insert(temp_constraints.end(), 
                              negated_constraints.begin(), 
                              negated_constraints.end());
        
        if (sat_solver(temp_constraints)) {
            output = temp_constraints;
            output.emplace_back(); 
            return true;
        }
    ...
    // Take the inverse logic of the constraint conditions (simplified example)[path constraints]
    vector<string> negate_constraints(const vector<string>& constraints) {
        vector<string> original_constraints = constraints;
        vector<string> negated_constraints = negate_constraints(original_constraints);
        vector<string> negated;
        for (const auto& cons : constraints) {
            if (cons.find("==") != string::npos) {
                negated.push_back(constraints.end());
            } else if (cons.find("<") != string::npos) {
                negated.push_back(constraints.end());
            }
        }
        return negated;
    ...
    // Simulate the SAT solver (return whether it is solvable)
    bool sat_solver(const vector<string>& constraints) {
        std::set<string> constraint_set(constraints.begin(), constraints.end());
		// The actual SAT solver (such as Z3) should be called here.
        for (const auto& cons : constraints) {
            string neg = negate_constraints({cons})[0];
            if (constraint_set.count(neg)) {
                return false;
            }
        }
        return true;
