...
extern std::vector<RTLIL::SigSpec> get_corresponding_label_signals(RTLIL::Module *module, std::vector<string> *excluded_signals,
								   const RTLIL::SigSpec &sig, unsigned int num_taints, unsigned int num_states);
...
extern bool cellift_add(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, unsigned int num_states, std::vector<string> *excluded_signals);
extern bool cellift_sub(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, std::vector<string> *excluded_signals);//
extern bool cellift_not(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, unsigned int num_states, std::vector<string> *excluded_signals);
extern bool cellift_neg(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, std::vector<string> *excluded_signals);
extern bool cellift_and(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, unsigned int num_states, std::vector<string> *excluded_signals);
extern bool cellift_or(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, unsigned int num_states, std::vector<string> *excluded_signals);
...
extern bool cellift_mux(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, unsigned int num_states, std::vector<string> *excluded_signals);
extern bool cellift_xor(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, unsigned int num_states, std::vector<string> *excluded_signals);
...
extern bool cellift_memv2(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_tiants, std::vector<string> *excluded_signals);
...
struct CellIFTWorker {
	...
	void create_cellift_logic()
	{
		...
		int loop_num = 0;
		int mod_num = 0;
		if(state_map.find("loop_num") != state_map.end())
			loop_num = state_map["loop_num"];
		if(state_map.find("mod_num") != state_map.end())
			mod_num = state_map["mod_num"];
		int num_states = loop_num + mod_num;
			...
			if (wire_it.second->port_input && !is_signal_excluded(excluded_signals, wire_it.first.str())) {
				for (unsigned int taint_id = 0; taint_id < num_taints; taint_id++) {
					input_wires_to_add.insert(
					  std::pair<RTLIL::IdString, int>(get_wire_taint_idstring(wire_it.first, taint_id), wire_it.second->width + num_states));
				}
			}
		...
		for (auto &cell : original_cells) {
			...
			////
			/*
			else if (cell->type.in(ID($memrd_v2))) {
				log("    	CellIFTing %s cell (%s)\n", cell->type.c_str(), cell->name.c_str());
				if (cell->module->get_bool_attribute(module_up_secret) || cell->module->get_bool_attribute(module_down_secret))
					keep_current_cell = cellift_memv2(module, cell, num_taints, excluded_signals);
				else
					log("This is a nomal memroy module, do not any operation on it.\n");
			}

			else if (cell->type.in(ID($meminit)))
				continue;
			*/
			...
			////
				...
				if (opt_conjunctive_cells_pool.find("add") != opt_conjunctive_cells_pool.end())
					...
					keep_current_cell = cellift_add(module, cell, num_taints, num_states, excluded_signals);

			else if (cell->type.in(ID($sub)))//not yet states
				...
				keep_current_cell = cellift_not(module, cell, num_taints, num_states, excluded_signals);

			else if (cell->type.in(ID($neg)))//not yet states
				...
				if (opt_conjunctive_cells_pool.find("and") != opt_conjunctive_cells_pool.end() || opt_conjunctive_gates)
					...
					keep_current_cell = cellift_and(module, cell, num_taints, num_states, excluded_signals);
				...
				if (opt_conjunctive_cells_pool.find("or") != opt_conjunctive_cells_pool.end() || opt_conjunctive_gates)
					...
					keep_current_cell = cellift_or(module, cell, num_taints, num_states, excluded_signals);
				...
				if (opt_conjunctive_cells_pool.find("mux") != opt_conjunctive_cells_pool.end())
					...
					keep_current_cell = cellift_mux(module, cell, num_taints, num_states, excluded_signals);
			// up-secret moudle OR down-secret module
			else if (cell->type.in(ID($xor), ID($_XOR_), ID($_XNOR_)))
				keep_current_cell = cellift_xor(module, cell, num_taints, num_states, excluded_signals);
				...
				for (auto &it : orig_ports) {
					...
					std::vector<RTLIL::SigSpec> port_taints =
					  get_corresponding_label_signals(module, excluded_signals, connected_sig, num_taints, state_map["state_num"]);
					
					/*
					if(connected_sig.chunks().size() > 1)
						port_taints.front().append(RTLIL::SigChunk(RTLIL::State::S0, num_states));
					*/
		...
		for (auto &conn : connections) {
			...
			std::vector<RTLIL::SigSpec> first = get_corresponding_label_signals(module, excluded_signals, conn.first, num_taints, state_map["state_num"]);
			std::vector<RTLIL::SigSpec> second = get_corresponding_label_signals(module, excluded_signals, conn.second, num_taints, state_map["state_num"]);

			/*
			//more to one  already append into get_corresponding_label_signals();
			if(second.front().chunks().size() > 1 && first.front().chunks().size() == 1){
				std::vector<RTLIL::SigChunk> taint_chunks = second.front().chunks();
				std::vector<RTLIL::SigChunk> data_chunks = conn.second.chunks();

				int width = data_chunks.front().width;
				RTLIL::SigSpec cmp_a = RTLIL::SigSpec(taint_chunks.front().extract(width, state_map["state_num"]));
				RTLIL::SigSpec cmp_b, cmp_y;
				RTLIL::SigSpec sele;
				int count;
				for(count = 1; count < taint_chunks.size(); count++)
				{
					width = data_chunks[count].width;
					cmp_b = RTLIL::SigSpec(taint_chunks[count].extract(width, state_map["state_num"]));
					//cmp_b = second[count].extract_end(conn.second.size());
					sele = module->Ge(NEW_ID, cmp_a, cmp_b);
					cmp_y = module->Mux(NEW_ID, cmp_a, cmp_b, sele);
					cmp_a = cmp_y;
				}
				second.front().append(cmp_a);
			}
			

			// one to more
			if(second.front().chunks().size() == 1 && second.front().as_chunk().wire != NULL && second.front().as_chunk().width < second.front().as_chunk().wire->width)
			{
					RTLIL::SigSpec expand = RTLIL::SigSpec(second.front().as_chunk().wire);
					expand = expand.extract_end(second.front().as_chunk().wire->width - state_map["state_num"]);
					//expand.extract(conn.second.size(), 4);
					second.front().remove(conn.second.size(), state_map["state_num"]);
					second.front().append(expand);
			}
			*/
	...
	CellIFTWorker(RTLIL::Module *_module, bool _opt_verbose, bool _opt_rtlift, bool _opt_conjunctive_gates,
		      ...
		      bool _opt_pmux_use_large_cells, int unsigned _num_taints, std::vector<string> *_excluded_signals)
	{
		module = _module;
...
struct CelliftPass : public Pass {
	...
	void execute(std::vector<std::string> args, RTLIL::Design *design) override
	{
		...
		for (auto i = 0; i < GetSize(topo_modules.sorted); ++i) {
			...
			/*
			log("module is %s, \nif it is up-secret module, print 1; \nif it is down-secret module, pirnt 2;  \n", module->name.c_str());
			
			scanf("%d", &attr);
			if (attr == 1) {
				module->set_bool_attribute(module_up_secret, true);
				if (module->get_bool_attribute(module_up_secret))
					printf("set %s  as a up_secret moudle\n", module->name.c_str());
				else
					printf("set up_secret attribute failed");

			}

			else if (attr == 2) {
				module->set_bool_attribute(module_down_secret, true);
				if (module->get_bool_attribute(module_down_secret))
					log("set %s as a down_secret module\n", module->name.c_str());
				else
					log("set down_secret attribute failed");
			} else
				log("Did not set any attribute on this module\n");

			// check wheather attribute set successfully
			//???
			//
			*/

			CellIFTWorker(module, opt_verbose, opt_rtlift, opt_conjunctive_gates, opt_conjunctive_cells_pool, opt_precise_shiftx,
				      opt_imprecise_shl_sshl, opt_imprecise_shr_sshr, opt_pmux_use_large_cells, num_taints, &opt_excluded_signals);
		}
	}
} CelliftPass;
