...
bool cellift_logic_and(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, std::vector<string> *excluded_signals) {
    ...
    if (ports[A].size() == 1 && ports[B].size() == 1) // Equivalent to traditional and operator.
        for (unsigned int taint_id = 0; taint_id < num_taints; taint_id++) {
            RTLIL::SigSpec a_taint_and_b = module->And(NEW_ID, port_taints[A][taint_id].extract(0, 1), ports[B]);
            RTLIL::SigSpec b_taint_and_a = module->And(NEW_ID, port_taints[B][taint_id].extract(0, 1), ports[A]);
            RTLIL::SigSpec a_taint_and_b_taint = module->And(NEW_ID, port_taints[A][taint_id].extract(0, 1), port_taints[B][taint_id].extract(0, 1));
        ...
        for (unsigned int taint_id = 0; taint_id < num_taints; taint_id++) {
            RTLIL::SigSpec not_a_taint = module->Not(NEW_ID, port_taints[A][taint_id].extract(0, ports[A].size()));
            RTLIL::SigSpec not_b_taint = module->Not(NEW_ID, port_taints[B][taint_id].extract(0, ports[B].size()));
            ...
            RTLIL::SigSpec max_a = module->Or(NEW_ID, ports[A], port_taints[A][taint_id].extract(0, ports[A].size()));
            RTLIL::SigSpec min_b = module->And(NEW_ID, ports[B], not_b_taint);
            RTLIL::SigSpec max_b = module->Or(NEW_ID, ports[B], port_taints[B][taint_id].extract(0, ports[B].size()));
            ...
            module->addXor(NEW_ID, min_and, max_and, port_taints[Y][taint_id]);
