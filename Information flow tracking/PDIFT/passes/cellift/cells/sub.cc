...
bool cellift_sub(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, std::vector<string> *excluded_signals) {
    ...
    for (unsigned int taint_id = 0; taint_id < num_taints; taint_id++) {
        ...
        if(cell->get_string_attribute(ID(const_opt)) == "B"){

        }
        else{
        RTLIL::SigSpec not_a_taint = module->Not(NEW_ID, extended_a_taint);
        //RTLIL::SigSpec not_b_taint = module->Not(NEW_ID, extended_b_taint);  1111111111111111

        RTLIL::SigSpec a_and_not_a_taint = module->And(NEW_ID, extended_a, not_a_taint);
        //RTLIL::SigSpec b_and_not_b_taint = module->And(NEW_ID, extended_b, not_b_taint); extended_b

        RTLIL::SigSpec a_or_a_taint = module->Or(NEW_ID, extended_a, extended_a_taint);
        //RTLIL::SigSpec b_or_b_taint = module->Or(NEW_ID, extended_b, extended_b_taint);  extended_b

        RTLIL::SigSpec a_one_minus_b_zero = module->Sub(NEW_ID, a_or_a_taint, extended_b);
        RTLIL::SigSpec a_zero_minus_b_one = module->Sub(NEW_ID, a_and_not_a_taint, extended_b);
        ...
        ////process the path\state label;

        ////end of process the path\state label;
        port_taints[Y][taint_id].append(RTLIL::SigSpec(port_taints[A][taint_id].extract_end(output_width)));
    }
