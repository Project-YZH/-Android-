 ...
 */
bool cellift_sdffe(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, std::vector<string> *excluded_signals) {

    const unsigned int CLK = 0, SRST = 1, EN = 2, D = 3, Q = 4;
    const unsigned int NUM_PORTS = 5;
    RTLIL::SigSpec ports[NUM_PORTS] = {cell->getPort(ID::CLK), cell->getPort(ID::SRST), cell->getPort(ID::EN), cell->getPort(ID::D), cell->getPort(ID::Q)};
    ...
    bool rst_polarity = cell->getParam(ID(SRST_POLARITY)).as_bool();
    ...
    for (unsigned int taint_id = 0; taint_id < num_taints; taint_id++) {
        ...
        reduce_or_arr[2] = module->And(NEW_ID, d_q_tainted_or_distinct, port_taints[EN][taint_id].extract(0, ports[2].size()));
        ...
        reduce_or_output.append(RTLIL::SigSpec(port_taints[D][taint_id].extract_end(data_width)));
        //reduce_or_output.append(RTLIL::SigSpec(RTLIL::State::S0, state_map["state_num"]));//taint
/*
...
*/
        ...
        RTLIL::Cell *new_ff = module->addSdff(NEW_ID, ports[CLK], ports[SRST], reduce_or_output, port_taints[Q][taint_id], RTLIL::Const(rst_val, data_width+state_map["state_num"]), clk_polarity, rst_polarity);
