...
bool cellift_adffe(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, std::vector<string> *excluded_signals) {
    ...
    for (unsigned int taint_id = 0; taint_id < num_taints; taint_id++) {
        ...
        reduce_or_arr[2] = module->And(NEW_ID, d_q_tainted_or_distinct, port_taints[EN][taint_id].extract(0, 1));
        ...
        int rst_val = 0;
        RTLIL::Cell *new_ff = module->addAdff(NEW_ID, ports[CLK], ports[ARST], reduce_or_output, port_taints[Q][taint_id], RTLIL::Const(rst_val, data_width+state_map["state_num"]), clk_polarity, rst_polarity);
