...
bool cellift_sdff(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, std::vector<string> *excluded_signals) {
    ...
    for (unsigned int taint_id = 0; taint_id < num_taints; taint_id++) {
        RTLIL::Cell *new_ff = module->addSdff(NEW_ID, ports[CLK], ports[SRST], port_taints[D][taint_id], port_taints[Q][taint_id], RTLIL::Const(srst_val, ports[Q].size()+state_map["state_num"]), clk_polarity, rst_lvl);
        if(cell->get_string_attribute(mod_flag) != "")
            printf("the sdff can't represent the exe flag of a module.\n");
