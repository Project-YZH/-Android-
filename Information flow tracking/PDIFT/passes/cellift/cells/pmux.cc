...
bool cellift_pmux_large_cells(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, std::vector<string> *excluded_signals) {
    ...
    for (unsigned int taint_id = 0; taint_id < num_taints; taint_id++) {
        ...
        // Is S tainted.
        RTLIL::SigSpec is_s_tainted = module->ReduceOr(NEW_ID, port_taints[S][taint_id].extract(0, s_size));
        ...
        RTLIL::SigSpec s_or_s_taint = module->Or(NEW_ID, ports[S], port_taints[S][taint_id].extract(0, s_size));
        // S taint-equal to zero.
        RTLIL::SigSpec not_s_taint = module->Not(NEW_ID, port_taints[S][taint_id].extract(0, s_size));
        ...
        // A
        RTLIL::SigSpec a_taint_and_s_taint_equal_zero = module->And(NEW_ID, port_taints[A][taint_id].extract(0, a_size), extended_s_taint_equal_zero_to_a_size);
        // B
        RTLIL::SigSpec b_taint_and_s_taint = module->And(NEW_ID, port_taints[B][taint_id].extract(0, b_size), extended_s_or_s_taint_b_size);
        // Fold the taint flow.
...
bool cellift_pmux_small_cells(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, std::vector<string> *excluded_signals) {
    ...
    if (s_size == 1) {
        ...
        for (unsigned int taint_id = 0; taint_id < num_taints; taint_id++) {
            RTLIL::SigSpec extended_s_taint = RTLIL::SigSpec(RTLIL::SigBit(port_taints[S][taint_id].extract(0, s_size)), data_size);
            ...
            RTLIL::SigSpec a_taint_and_not_s_or_s_taint = module->And(NEW_ID, port_taints[A][taint_id].extract(0, a_size), not_s_or_s_taint);
            RTLIL::SigSpec b_taint_and_s_or_s_taint = module->And(NEW_ID, port_taints[B][taint_id].extract(0, b_size), s_or_s_taint);
            ...
            // Output taint.
            module->addOr(NEW_ID, s_taint_and_a_xor_b, data_taint_stream, port_taints[Y][taint_id].extract(0, a_size));

            module->connect(port_taints[Y][taint_id].extract_end(data_size), port_taints[A][taint_id].extract_end(a_size));
        }
        return true;
    }
    ...
    for (unsigned int taint_id = 0; taint_id < num_taints; taint_id++) {
        ...
        module->addOr(NEW_ID, or_ab_bb, folded_or_b_taint_flow[s_size-1], port_taints[Y][taint_id].extract(0, a_size));
        module->connect(port_taints[Y][taint_id].extract_end(a_size), port_taints[A][taint_id].extract_end(a_size));
    }
