 ...
 */
bool cellift_add(RTLIL::Module *module, RTLIL::Cell *cell, unsigned int num_taints, unsigned int num_states, std::vector<string> *excluded_signals) {
    ...
    for (unsigned int taint_id = 0; taint_id < num_taints; taint_id++) {
        ...
        //?       //port_taints[A][taint_id].extract_end(output_width)
        ...
        RTLIL::SigSpec a_plus_b_not_taints = module->Add(NEW_ID, a_and_not_a_taint, extended_b);
        RTLIL::SigSpec a_plus_b_or_taints = module->Add(NEW_ID, a_or_a_taint, extended_b);
