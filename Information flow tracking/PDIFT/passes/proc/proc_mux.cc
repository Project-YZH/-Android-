...
RTLIL::SigSpec gen_mux(RTLIL::Module *mod, const RTLIL::SigSpec &signal, const std::vector<RTLIL::SigSpec> &compare, RTLIL::SigSpec when_signal, RTLIL::SigSpec else_signal, RTLIL::Cell *&last_mux_cell, RTLIL::SwitchRule *sw, RTLIL::CaseRule *cs, bool ifxmode, bool flag_opt)
{
	...
	//add optimize ooperation
	///*[2024.11.28]
	if(flag_opt == true)
		mux_cell->set_bool_attribute(ID(untaint_area), true);
	//[2024.11.28]*/

	/*
	if(else_signal.is_fully_const() && when_signal.is_fully_const())
		mux_cell->set_string_attribute(ID(opt_flag), "2");
	else if(else_signal.is_fully_const() && !when_signal.is_fully_const())
		mux_cell->set_string_attribute(ID(opt_flag),"1-A");
	else if(!else_signal.is_fully_const() && when_signal.is_fully_const())
		mux_cell->set_string_attribute(ID(opt_flag),"1-B");		
	else
		mux_cell->set_string_attribute(ID(opt_flag),"0");	*/
...
RTLIL::SigSpec signal_to_mux_tree(RTLIL::Module *mod, SnippetSwCache &swcache, dict<RTLIL::SwitchRule*, bool, hash_ptr_ops> &swpara,
		RTLIL::CaseRule *cs, const RTLIL::SigSpec &sig, const RTLIL::SigSpec &defval, bool ifxmode, bool flag_opt)
{
	...
	for (auto sw : cs->switches)
	{
		...
		for (size_t i = 0; i < sw->cases.size(); i++) {
			...
			RTLIL::SigSpec value = signal_to_mux_tree(mod, swcache, swpara, cs2, sig, initial_val, ifxmode, flag_opt);
			if (last_mux_cell && pgroups[case_idx] == pgroups[case_idx+1])
				...
				result = gen_mux(mod, sw->signal, cs2->compare, value, result, last_mux_cell, sw, cs2, ifxmode, flag_opt);
		}
	}
...
void proc_mux(RTLIL::Module *mod, RTLIL::Process *proc, bool ifxmode)
{
	...
	for (int idx : sigsnip.snippets)
	{
		...
		////SHIFT 2024.11.13
		bool flag_opt = false;
		if(proc->get_bool_attribute(ID(untaint_area)) == true)
			flag_opt = true;
		RTLIL::SigSpec value = signal_to_mux_tree(mod, swcache, swpara, &proc->root_case, sig, RTLIL::SigSpec(RTLIL::State::Sx, sig.size()), ifxmode, flag_opt);
