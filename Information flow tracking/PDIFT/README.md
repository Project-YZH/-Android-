...
External Dependency: Berkeley ABC
=================================

Yosys heavily relies on **Berkeley ABC** for logic synthesis and optimization. The source code for ABC is expected to reside in the `abc/` subdirectory.

If the `abc` directory is empty or missing (which causes the build to fail with errors related to `yosys-abc`), it is likely because the git submodules were not initialized.

**How to fetch the missing `abc` directory:**

1.  **Using Git (Recommended)**
    Run this command in the Yosys root directory to download the correct version of ABC:
    
    ```bash
    git submodule update --init --recursive
    ```

2.  **Manual Download (If Git fails)**
    If you cannot use git submodules, you must download the ABC source code manually and place it in the `abc/` directory. 
    
    *Note: Yosys requires a specific version of ABC. Using the latest version might cause compatibility issues.*
    
    ```bash
    # Clone ABC into the 'abc' directory
    git clone https://github.com/berkeley-abc/abc.git abc
    
    # Switch to the specific commit hash used by this version of Yosys
    # (Check the .gitmodules file or Yosys commit logs to find the hash, e.g., ab5b16e)
    cd abc
    git checkout ab5b16e
    cd ..
    ```

**Building ABC:**
You do **not** need to compile ABC manually inside the `abc` folder. Once the directory is populated with the source code (containing the `Makefile`), simply run `make` in the **Yosys root directory**. The Yosys build system will automatically invoke the ABC Makefile, compile it, and link it as `yosys-abc`.
